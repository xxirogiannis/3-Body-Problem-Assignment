...
ax.set_xlim(-2, 2) # Προσάρμοσε τα όρια ανάλογα με τις τροχιές σου
ax.set_ylim(-2, 2)
ax.set_aspect('equal')

# Δημιουργία των αντικειμένων που θα "κινούνται"
line1, = ax.plot([], [], 'r-', lw=1, label="Body 1")
line2, = ax.plot([], [], 'g-', lw=1, label="Body 2")
line3, = ax.plot([], [], 'b-', lw=1, label="Body 3")

# Σημεία για να φαίνονται οι τρέχουσες θέσεις των σωμάτων
dot1, = ax.plot([], [], 'ro')
dot2, = ax.plot([], [], 'go')
dot3, = ax.plot([], [], 'bo')
...
# Συνάρτηση που καλείται σε κάθε frame
def update(frame):
    # Παίρνουμε τα δεδομένα μέχρι το τρέχον frame
    line1.set_data(y[:frame, 0], y[:frame, 1])
    line2.set_data(y[:frame, 3], y[:frame, 4])
    line3.set_data(y[:frame, 6], y[:frame, 7])
    
    # Ενημέρωση της τρέχουσας θέσης (τελευταίο σημείο)
...
# interval: καθυστέρηση μεταξύ frames σε milliseconds
